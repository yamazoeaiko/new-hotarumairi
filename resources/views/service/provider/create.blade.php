  ...
  <form action="{{ route('service.create.done') }}" method="post" class="form-control" enctype="multipart/form-data">
    ...
    <div class="mb-3">
      ...
      <div class="input-group">
        <textarea onkeydown="if(event.keyCode == 13 && !event.shiftKey){event.preventDefault(); this.value += '\n';}" name="content" class="text-start input-group-text is-valid" style="resize: none; height: 70px; overflow-y: auto; padding: 10px; width: 100%;" oninput="this.style.height = '70px'; this.style.height = (this.scrollHeight + 10) + 'px';" placeholder="サービス内容、購入者のメリット、アピールポイントなどを具体的にご記載ください。">{{old('content')}}</textarea>
      </div>
    ...
    <div class="mb-3">
      ...
      <div class="input-group">
        <textarea onkeydown="if(event.keyCode == 13 && !event.shiftKey){event.preventDefault(); this.value += '\n';}" name="attention" class="text-start input-group-text is-valid" style="resize: none; height: 70px; overflow-y: auto; padding: 10px; width: 100%;" oninput="this.style.height = '70px'; this.style.height = (this.scrollHeight + 10) + 'px';" placeholder="購入前に教えて欲しいことなどがあれば、ご記載ください。（任意）">{{old('attention')}}</textarea>
      </div>
    </div>
