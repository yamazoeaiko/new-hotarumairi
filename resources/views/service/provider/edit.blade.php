...
<div class="mb-3">
  ...
  <div class="input-group">
    <textarea name="attention" class="text-start input-group-text is-valid" style="resize: none; height: 150px; overflow-y: auto; padding: 10px; width: 100%;" oninput="this.style.height = '70px'; this.style.height = (this.scrollHeight + 10) + 'px';" placeholder="購入前に教えて欲しいことなどがあれば、をご記載ください。（任意）">{{ $item->attention }}</textarea>
  </div>
</div>
...
<div class="form-group mb-3">
  ...
  <input type="date" class="form-control" name="application_deadline" value="{{$item->application_deadline}}" min="{{ date('Y-m-d') }}">
</div>

<div class="form-group mb-3">
  <label class="fw-bolder" for="delivery_deadline">希望納品（実施）日<span class="fs-7 text-secondary">※任意</span></label>
  <input type="date" class="form-control" name="delivery_deadline" value="{{$item->delivery_deadline}}" min="{{ date('Y-m-d') }}">
</div>
