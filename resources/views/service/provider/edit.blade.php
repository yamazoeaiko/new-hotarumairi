...
<div class="container">
  <form action="{{ route('mypage.service.update') }}" method="post" class="form-control" enctype="multipart/form-data">
    ...
    <div class="mb-3">
      ...
      <input type="text" name="main_title" class="form-control fw-bolder" value="{{ $item->main_title }}" maxlength="20">
    ...
    <div class="mb-3">
      ...
      <div class="input-group">
        <textarea oninput="resizeTextarea(this)" name="content" class="text-start form-control" style="resize: none; height: 150px; overflow-y: auto; padding: 10px; width: 100%;" oninput="this.style.height = '150px'; this.style.height = (this.scrollHeight + 10) + 'px';" placeholder="サービス内容、購入者のメリット、アピールポイントなどを具体的にご記載ください。">{{ $item->content }}</textarea>
      </div>
    ...
    <div class="mb-3">
      ...
      <div class="input-group">
        <textarea oninput="resizeTextarea(this)" name="attention" class="text-start form-control" style="resize: none; height: 150px; overflow-y: auto; padding: 10px; width: 100%;" oninput="this.style.height = '70px'; this.style.height = (this.scrollHeight + 10) + 'px';" placeholder="購入前に教えて欲しいことなどがあれば、ご記載ください。（任意）">{{ $item->attention }}</textarea>
      </div>
    </div>
    ...
    <div class="form-group mb-3">
      ...
      <textarea class="form-control" name="free" oninput="resizeTextarea(this)" oninput="this.style.height = '70px'; this.style.height = (this.scrollHeight + 10) + 'px';" placeholder="その他なにかあればご自由にご記載ください。（任意）">{{$item->free}}</textarea>
    </div>
