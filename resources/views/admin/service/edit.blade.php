  ...
  <div class="container">
    <form action="{{ route('admin.service.update') }}" method="post" class="form-control" enctype="multipart/form-data">
      ...
      <div class="mb-3">
        ...
        <div class="input-group">
          <textarea onkeydown="if(event.keyCode == 13 && !event.shiftKey){event.preventDefault(); this.value += '\n';}" name="attention" class="text-start input-group-text is-valid" style="resize: none; height: 150px; overflow-y: auto; padding: 10px; width: 100%;" oninput="this.style.height = '70px'; this.style.height = (this.scrollHeight + 10) + 'px';" placeholder="購入前に教えて欲しいことなどがあれば、ご記載ください。（任意）">{{ $item->attention }}</textarea>
        </div>
      </div>
      ...
      <div class="form-group mb-3">
        ...
        <textarea onkeydown="if(event.keyCode == 13 && !event.shiftKey){event.preventDefault(); this.value += '\n';}" oninput="this.style.height = '70px'; this.style.height = (this.scrollHeight + 10) + 'px';" placeholder="Enterで改行されます。" class="form-control" name="free">{{$item->free}}</textarea>
      </div>
